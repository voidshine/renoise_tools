...
import * as doc from './renoise_document';
...
export namespace Song {
    const MAX_NUMBER_OF_INSTRUMENTS: number;
}
...
describe_undo(description: string): void;
...
insert_track_at(index: LuaIndex): Track;
...
delete_track_at(index: LuaIndex): void;
...
swap_tracks_at(index1: LuaIndex, index2: LuaIndex): void;
...
select_previous_track(): void;

//     renoise.song():select_next_track()
select_next_track(): void;
...
capture_instrument_from_pattern(): void;
...
capture_nearest_instrument_from_pattern(): void;
...
pattern(index: LuaIndex): Pattern;
...
readonly rendering_progress: number;
...
readonly selected_instrument: Instrument;

//     renoise.song().selected_instrument_index, _observable
//       -> [number]
selected_instrument_index: LuaIndex;
...
selected_phrase_index: LuaIndex;
...
selected_track_observable: doc.renoise.Document.Observable;

//     renoise.song().selected_track_index, _observable
//       -> [number]
selected_track_index: LuaIndex;
selected_track_index_observable: doc.renoise.Document.Observable;
    
//     -- Selected in the track DSP chain editor. Can be nil.
//     renoise.song().selected_track_device, _observable
//       -> [read-only, renoise.AudioDevice object or nil]
selected_track_device: AudioDevice;

//     renoise.song().selected_track_device_index
//       -> [number, index or 0 (when no device is selected)]
selected_track_device_index: LuaIndex;
...
//       -> [number]
selected_pattern_index: LuaIndex;
...
// TODO: Is this an @tupleReturn property?
readonly selected_note_column: NoteColumn | null;

//     renoise.song().selected_note_column_index
//       -> [number, index or 0 (when an effect column is selected)]
selected_note_column_index: LuaIndex;
...
selected_effect_column_index: LuaIndex;
...
selection_in_pattern: PatternSelection | null;
...
export namespace Transport {
    ...
    /** @compileMembersOnly */
    export enum RECORD_PARAMETER_MODE {
        RECORD_PARAMETER_MODE_PATTERN,
        RECORD_PARAMETER_MODE_AUTOMATION,
    }

    /** @compileMembersOnly */
    export enum TIMING_MODEL {
        TIMING_MODEL_SPEED,
        TIMING_MODEL_LPB,
    }    
}
...
trigger_sequence(sequence_pos: LuaIndex): void;
...
readonly timing_model: Transport.TIMING_MODEL;
...
//       -> [read-only, SongPos]
readonly song_length: SongPos;

//     renoise.song().transport.song_length_beats
//       -> [read-only, number]
readonly song_length_beats: number;
...
readonly loop_block_start_pos: SongPos;
//     renoise.song().transport.loop_block_range_coeff
//       -> [number, 2-16]
loop_block_range_coeff: number;
...
insert_sequence_at(sequence_pos: LuaIndex, pattern_index: LuaIndex): void;
...
insert_new_pattern_at(sequence_pos: LuaIndex): LuaIndex;
...
delete_sequence_at(sequence_pos: LuaIndex): void;
...
pattern(sequence_pos: LuaIndex): LuaIndex;
...
clone_range(from_sequence_pos: LuaIndex, to_sequence_pos: LuaIndex): void;
...
make_range_unique(from_sequence_pos: LuaIndex, to_sequence_pos: LuaIndex): void;
...
//       -> [boolean]
sequence_is_start_of_section(sequence_index: LuaIndex): boolean;

//     renoise.song().sequencer:set_sequence_is_start_of_section(
//       sequence_index, true_or_false) 
set_sequence_is_start_of_section(sequence_index: LuaIndex, true_or_false: boolean): void;
...
//       -> [string]
sequence_section_name(sequence_index: LuaIndex): string;

//     renoise.song().sequencer:set_sequence_section_name(sequence_index, string)
set_sequence_section_name(sequence_index: LuaIndex, name: string): void;
...
//       -> [boolean]
sequence_is_part_of_section(sequence_index: LuaIndex): boolean;
...
//       -> [boolean]
sequence_is_end_of_section(sequence_index: LuaIndex): boolean;
...
//       -> [boolean]
track_sequence_slot_is_muted(track_index: LuaIndex, sequence_index: LuaIndex): boolean;

//     renoise.song().sequencer:set_track_sequence_slot_is_muted(
//       track_index, sequence_index, muted)
set_track_sequence_slot_is_muted(track_index: LuaIndex, sequence_index: LuaIndex, muted: boolean): void;
...
//       -> [boolean]
track_sequence_slot_is_selected(track_index: LuaIndex, sequence_index: LuaIndex): boolean;

//     renoise.song().sequencer:set_track_sequence_slot_is_selected(
//       track_index, sequence_index, selected)
set_track_sequence_slot_is_selected(track_index: LuaIndex, sequence_index: LuaIndex, selected: boolean): void;
...
//       -> [renoise.DeviceParameter object]
postfx_volume: DeviceParameter;

//     renoise.song().tracks[].postfx_panning
//       -> [renoise.DeviceParameter object]
postfx_panning: DeviceParameter;
...
//       -> [read-only, number]
readonly value_quantum: number;

//     renoise.song().tracks[].devices[].parameters[].value_default
//       -> [read-only, number]
readonly value_default: number;
...
//     -- Phrase const lines per beat setting. New phrases get initialized with 
...
export namespace Pattern {
    const MAX_NUMBER_OF_LINES: number;
}
...
track(index: LuaIndex): PatternTrack;
...
// TODO: Is this extends true? Mostly. Example difference: color properties have different types: this one is nullable.
// No! Example: PatternTrack does not have name property. Nor visible_note_columns.
...
copy_from(other: PatternTrack): void;
...
//       -> [renoise.PatternLine]
line(index: LuaIndex): PatternLine;
...
//       -> [array of renoise.PatternLine objects]
lines_in_range(index_from: LuaIndex, index_to: LuaIndex): PatternLine[];
...
copy_from(other: PatternLine): void;
...
note_column(index: LuaIndex): NoteColumn;
...
effect_column(index: LuaIndex): EffectColumn;    
...
// TODO: Are contents Readonly<> as well?
readonly note_columns: NoteColumn[];

//     renoise.song().patterns[].tracks[].lines[].effect_columns[]
//       -> [read-only, array of renoise.EffectColumn objects]
readonly effect_columns: EffectColumn[];
...
copy_from(other: NoteColumn): void;
...
export interface PatternSelection {
    start_line: LuaIndex;     // Start pattern line index
    start_track: LuaIndex;    // Start track index
    start_column: LuaIndex;   // Start column index within start_track   
    end_line: LuaIndex;       // End pattern line index
    end_track: LuaIndex;      // End track index
    end_column: LuaIndex;     // End column index within end_track
}
